...
Image_gamma_correct(int argc, VALUE *argv, VALUE self)
{
    ...
    {
#if defined(IMAGEMAGICK_7)
        SET_CHANNEL_MASK(new_image, (ChannelType) (RedChannel | GreenChannel | BlueChannel));
    ...
    {
#if defined(IMAGEMAGICK_7)
        SET_CHANNEL_MASK(new_image, RedChannel);
        (void) GammaImage(new_image, red_gamma, exception);
        RESTORE_CHANNEL_MASK(new_image);

        SET_CHANNEL_MASK(new_image, GreenChannel);
        (void) GammaImage(new_image, green_gamma, exception);
        RESTORE_CHANNEL_MASK(new_image);

        SET_CHANNEL_MASK(new_image, BlueChannel);
...
#if defined(IMAGEMAGICK_7)
    draw_info->fill.alpha = alpha_val;
#else
